...
export class AudioEngine {
    ...
    createTrackChannels(tracks: Track[]) {
        ...
        tracks.forEach(track => {
            ...
            const gain = this.audioContext.createGain(); gain.gain.value = track.volume;
    ...
    triggerRelease(param: AudioParam, time: number, env: Envelope, baseValue = 0) {
        ...
        param.setTargetAtTime(targetValue, time, Math.max(epsilon, release / 4));
    }
    ...
    private shouldTrigger(trackId: number, step: StepState, loopCount: number) {
        // More robust guard against malformed step.condition objects from older project versions
        ...
        switch (type) {
            case 'probability': return Math.random() * 100 < p!;
            case 'a:b': return (count % b!) + 1 === a!;
    ...
    private createKickVoice(t: Track, p: PLocks | null, time: number, note: string, vel: number, lt: number) {
        ...
        const cEnv = { attack: 0.001, decay: 0.015, sustain: 0, release: 0.01 };
    ...
    private createArcaneVoice(t: Track, p: PLocks | null, time: number, note: string, vel: number, lt: number) { const limiter = this.trackLimiters.get(t.id); if (!limiter) return; const param = this.createParameterResolver(t, p, lt); const freq = noteToFreq(note); if(!isFinite(freq)) return; const rawEnv = param('ampEnv'); const env = { attack: finite(rawEnv?.attack, 0.01), decay: finite(rawEnv?.decay, 0.4), sustain: finite(rawEnv?.sustain, 0.2), release: finite(rawEnv?.release, 0.3) }; const stopTime = time + env.attack + env.decay + env.release + 0.5; const panner = this.audioContext.createStereoPanner(); const finalGain = this.audioContext.createGain(); const amp = this.audioContext.createGain(); const o1 = this.audioContext.createOscillator(); const o1s = this.audioContext.createWaveShaper(); const o1g = this.audioContext.createGain(); const o2 = this.audioContext.createOscillator(); const o2s = this.audioContext.createWaveShaper(); const o2g = this.audioContext.createGain(); const mod = this.audioContext.createGain(); const fold = this.audioContext.createWaveShaper(); const f = this.audioContext.createBiquadFilter(); let rm: GainNode | null = null; const nodes: AudioNode[] = [panner, finalGain, amp, o1, o1s, o1g, o2, o2s, o2g, mod, fold, f]; o1.connect(o1s).connect(o1g); o2.connect(o2s).connect(o2g); o2g.connect(mod); const mode = param('mode'); switch (mode) { case 'pm': mod.connect(o1.frequency); break; case 'add': o1g.connect(amp); o2g.connect(amp); break; case 'ring': rm = this.audioContext.createGain(); nodes.push(rm); o1g.connect(rm.gain); mod.connect(rm); rm.connect(amp); break; case 'hard_sync': mod.connect(o1.frequency); break; } if (mode !== 'add' && mode !== 'ring') o1g.connect(amp); amp.connect(fold).connect(f).connect(finalGain).connect(panner).connect(limiter); this.applyEnvelope(amp.gain, time, env, vel); this.triggerRelease(amp.gain, time + env.attack + env.decay, env); panner.pan.setValueAtTime(0, time); finalGain.gain.value = 0.251; o1.frequency.setValueAtTime(freq, time); o1s.curve = this.getArcaneShapeCurve(finite(param('osc1_shape'), 50)); const spread = finite(param('spread'), 10); o1.detune.setValueAtTime(spread / 2, time); o2.frequency.setValueAtTime(freq, time); o2.detune.setValueAtTime(finite(param('osc2_pitch'), 7) * 100 + finite(param('osc2_fine'), 5) - (spread / 2), time); o2s.curve = this.getArcaneShapeCurve(finite(param('osc2_shape'), 75)); mod.gain.setValueAtTime(finite(param('mod_amount'), 30) * (mode === 'pm' ? 50 : 1), time); fold.curve = this.makeFoldCurve(finite(param('fold'), 20), 1.0); f.type = param('filter.type') || 'lowpass'; f.frequency.setValueAtTime(finite(param('filter.cutoff'), 5000), time); f.Q.setValueAtTime(finite(param('filter.resonance'), 4), time); const lfo1 = param('lfo1'), lfo2 = param('lfo2'); const lfoTargets: Record<string, { param: AudioParam | AudioParam[], scale: number }> = { 'volume': { param: amp.gain, scale: 1 / 1000 }, 'pan': { param: panner.pan, scale: 1 / 1000 }, 'filter.cutoff': { param: f.frequency, scale: 10 }, 'filter.resonance': { param: f.Q, scale: 30 / 1000 }, 'arcane.osc2_pitch': { param: o2.detune, scale: 1 }, 'arcane.mod_amount': { param: mod.gain, scale: mode === 'pm' ? 50 : 1 }, 'arcane.spread': { param: o1.detune, scale: 0.5 / 1000 } }; [lfo1, lfo2].forEach(lfo => { const target = lfo && lfoTargets[lfo.destination]; if (target) nodes.push(...this.applyLFO(lfo, target.param, target.scale, time, stopTime)); }); o1.start(time); o2.start(time); o1.stop(stopTime); o2.stop(stopTime); return { nodes, stopTime }; }
    private createRuinVoice(t: Track, p: PLocks | null, time: number, note: string, vel: number, lt: number) {
        ...
        if (algorithm === 'feedback_pm') {
            ...
            nodes.push(fb);
            fb.gain.setValueAtTime(timbre / 100, time);
            
            // Routing for feedback PM
        ...
        // Set default gain for modulation nodes
        timbreGain.gain.setValueAtTime(1, time);
        driveGain.gain.setValueAtTime(1, time);
        foldGain.gain.setValueAtTime(1, time);
        ...
        const lfoTargets: Record<string, { param: AudioParam | AudioParam[], scale: number }> = {
            ...
            'ruin.timbre': { param: (algorithm === 'feedback_pm' && fb) ? fb.gain : timbreGain.gain, scale: (algorithm === 'feedback_pm' && fb) ? 1/1000 : 0.5/1000 },
            'ruin.drive': { param: driveGain.gain, scale: 0.5 / 1000 },
            'ruin.fold': { param: foldGain.gain, scale: 0.5 / 1000 },
        };
